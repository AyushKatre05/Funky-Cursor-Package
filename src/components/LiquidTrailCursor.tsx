...
const LiquidTrailCursor: React.FC<LiquidTrailProps> = ({
  color = "#1E90FF",
  size = 6,
  trails = 12,
  speed = 1.5,
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      update() {
        this.x += this.vx;
        this.y += this.vy;
        this.vx += (Math.random() - 0.5) * liquidFlow;
        this.vy += (Math.random() - 0.5) * liquidFlow;
        this.vx *= 0.98;
        this.vy *= 0.98;
      }
  ...
  }, [color, size, trails, speed, lifespan, liquidFlow]);

  return <canvas ref={canvasRef} className="fixed top-0 left-0 w-full h-full pointer-events-none" />;
};

export default LiquidTrailCursor;
